...
Discovery is read-only. Unknown files are ignored. A bad line does not invalidate valid lines in the
same file. Lines over the safety limit are skipped. Index titles are never included in the
canonical model.
...
scan. Files in legacy layouts without an identity in their filename remain fully scanned.
...
| `timestamp` | observed source timestamp or compatible index fallback |
...
The adapter hashes the complete parsed record for duplicate detection. A duplicate is removed only
when its hash and normalized session identity both match. This permits identical events in distinct
sessions while collapsing replayed events across rollout files belonging to one session.
...
Candidates are ordered by timestamp and then by source order. Exact normalized subjects correlate.
Tasks, approvals, blockers, and deployments may also correlate when their subject-token Jaccard
...
When a recorded working directory still belongs to a Git repository, the optional probe runs:
...
The environment disables optional Git locks. No hooks, commits, fetches, pushes, deployments, or
remote API calls are performed. Probe evidence is marked `repository_probe` and located at an opaque
`repo-…#current` reference.
...
default redactor and length bound before entering the serializable model.
...
do not imply freshness: `generated_at` identifies the observation time, and an explicit scan or
